...
describe('Myers Algorithm Memory Cleanup', () => {
  ...
  let progressCallback: ReturnType<typeof vi.fn<(progress: number, stage: string) => void>>;
  ...
  describe('Memory Cleanup Effectiveness', () => {
    ...
    it('should handle ESC key cancellation scenario', async () => {
      ...
      try {
        ...
        // Clear the timeout to prevent lingering operations
  ...
  describe('State Memory Cleanup (Requirements 2.1, 2.2, 2.3)', () => {
    // Add cleanup for hook tests to prevent uncaught cancellation errors
    afterEach(async () => {
      // Clear any global abort signals that might be lingering
      try {
    ...
    it('should include result: null in setState call when cancelComparison is called', async () => {
        ...
        if (result.current.isProcessing) {
          ...
          expect(result.current.error === 'Comparison cancelled by user' || result.current.error === '👍 Comparison cancelled successfully').toBe(true);
        } else {
          // If comparison completed too quickly, test explicit cancellation
          expect(typeof result.current.cancelComparison).toBe('function');
          
          // Test cancellation after completion
          act(() => {
            result.current.cancelComparison();
          });
          ...
          // After explicit cancellation, should have null result
          expect(result.current.result).toBeNull();
          expect(result.current.error === 'Comparison cancelled by user' || result.current.error === '👍 Comparison cancelled successfully').toBe(true);
        }
    ...
    describe('Requirement 2.2: Previous Results Cleared During Cancellation', () => {
      it('should clear previous comparison results when cancellation occurs', async () => {
        ...
        act(() => {
          result.current.setOriginalText('new original text that should take some time to process');
          result.current.setRevisedText('new revised text that should also take time to process');
          result.current.compareDocuments();
        });
        
        // Wait a tiny bit for processing to potentially start
        await act(async () => {
          await new Promise(resolve => setTimeout(resolve, 10));
        });
