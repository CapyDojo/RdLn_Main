// Test script to verify nuclear override fixes opacity while maintaining hover
console.log('🧪 NUCLEAR OVERRIDE FIX TEST');
...
if (inputPanels.length > 0) {
  ...
  // Test 1: Check base styles (should now have Kyoto dark background with nuclear override)
  console.log('\n📋 TEST 1: BASE STYLES (NUCLEAR OVERRIDE)');
  ...
  if (hasKyotoBackground) {
    console.log('✅ SUCCESS! Kyoto dark background detected (nuclear override working)');
  } else {
    console.log('❌ FAILURE! Still showing default theme background');
    console.log('Expected: rgba(28, 25, 23, ...) but got:', baseStyle.background);
  }
  ...
  const directHoverHandler = () => {
    ...
    if (hasStrongShadow && hasTransform) {
      console.log('✅ SUCCESS! Direct hover working with nuclear override!');
    } else {
  ...
  setTimeout(() => {
    ...
    if (hasHandleStrongShadow && hasHandleTransform) {
      console.log('✅ SUCCESS! Handle hover working with nuclear override!');
    } else {
    ...
    setTimeout(() => {
      ...
      if (hasForceStrongShadow && hasForceTransform) {
        console.log('✅ SUCCESS! Force-hover working with nuclear override!');
      } else {
      ...
      console.log('\n🎯 FINAL SUMMARY:');
      console.log('- Base glassmorphism opacity: ' + (hasKyotoBackground ? '✅ VIVID (NUCLEAR OVERRIDE)' : '❌ WHITISH'));
      ...
      console.log('- Nuclear override approach used to defeat glassmorphism.css !important');
